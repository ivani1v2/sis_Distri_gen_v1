    ...
    <v-dialog v-model="dialog" max-width="600" persistent>
        <v-card class="rounded-lg">
            <v-toolbar color="primary" dark dense height="48">
                <v-toolbar-title class="text-subtitle-1">Registrar Crédito Manual</v-toolbar-title>
                <v-spacer></v-spacer>
            ...
            <v-card-text class="pa-3">
                <v-row dense>
                    <v-col cols="8">
                        <v-autocomplete v-model="clienteSeleccionado" :items="clientes" :filter="customFilter"
                            item-text="nombre" item-value="id" label="Cliente *" outlined dense hide-details="auto"
                            class="mb-2" prepend-inner-icon="mdi-account-search" :loading="cargandoClientes"
                            return-object clearable @input="clienteChange" :search-input.sync="busquedaCliente">
                            <template v-slot:item="{ item }">
                                <v-list-item-content class="py-1">
                                    <v-list-item-title class="text-body-2">{{ item.nombre }}</v-list-item-title>
                                    <v-list-item-subtitle class="caption">{{ item.documento || item.id
                    ...
                    <v-col cols="4" class="d-flex align-center">
                        <v-chip small color="blue lighten-4" class="font-weight-medium">
                            <v-icon left small>mdi-map-marker</v-icon>
                            <v-progress-circular v-if="buscandoClientes" indeterminate size="12" width="2"
                                class="mr-1"></v-progress-circular>
                            Zona: {{ buscandoClientes ? 'buscando...' : zonaCliente }}
                        </v-chip>
                    </v-col>
                    <v-col cols="6">
                        <v-text-field v-model="documentoCliente" label="Documento" outlined dense hide-details="auto"
                            class="mb-2" :disabled="true" prepend-inner-icon="mdi-card-account-details"
                            readonly></v-text-field>
                    </v-col>

                    <v-col cols="6">
                        <v-select v-model="vendedorSeleccionado" :items="vendedoresItems" item-text="nombre"
                            item-value="codigo" label="Vendedor" outlined dense hide-details="auto" class="mb-2"
                            prepend-inner-icon="mdi-account-tie" clearable></v-select>
                    </v-col>

                    <v-col cols="6">
                        <v-text-field v-model="docRef" label="DOC Referencia" outlined dense hide-details="auto"
                            class="mb-2" prepend-inner-icon="mdi-identifier" placeholder="Ej: BD97-12345678"
                            :error="docRefError" :error-messages="docRefErrorMsg"
                            @input="docRef = $event.toUpperCase(); validarDocRef()"
                    ...
                    <v-col cols="6">
                        <v-menu v-model="menuFechaEmision" :close-on-content-click="false" transition="scale-transition"
                            offset-y max-width="290px" min-width="290px">
                            <template v-slot:activator="{ on, attrs }">
                                <v-text-field v-model="fechaEmisionStr" label="Emisión *"
                                    prepend-inner-icon="mdi-calendar" readonly v-bind="attrs" v-on="on" outlined dense
                                    hide-details="auto" class="mb-2"></v-text-field>
                            </template>
                            <v-date-picker v-model="fechaEmisionStr" @input="menuFechaEmision = false"
                                locale="es-pe"></v-date-picker>
                        </v-menu>
                    </v-col>

                    <v-col cols="6">
                        <v-menu v-model="menuFechaVence" :close-on-content-click="false" transition="scale-transition"
                            offset-y max-width="290px" min-width="290px">
                            <template v-slot:activator="{ on, attrs }">
                                <v-text-field v-model="fechaVenceStr" label="Vence *"
                                    prepend-inner-icon="mdi-calendar-clock" readonly v-bind="attrs" v-on="on" outlined
                                    dense hide-details="auto" class="mb-2"></v-text-field>
                            </template>
                            <v-date-picker v-model="fechaVenceStr" @input="menuFechaVence = false"
                                locale="es-pe"></v-date-picker>
                        </v-menu>
                    </v-col>
                    ...
                    <v-col cols="12">
                        ...
                        <v-alert
                            v-if="montoTotal && parseFloat(montoTotal) > 0 && (!cronogramaData || sumaCuotas === 0)"
                        ...
                        <v-alert v-if="cronogramaData && Math.abs(parseFloat(montoTotal || 0) - sumaCuotas) > 0.01"
                            type="warning" dense class="mb-2">
                            <div class="text-caption">
                                Descuadre: {{ monedaSeleccionada }}{{ redondear(parseFloat(montoTotal || 0) -
                                    sumaCuotas) }}
                            </div>
                        </v-alert>
                            ...
                            <template v-slot:default>
                                ...
                                <tbody>
                                    <tr v-for="(cuota, index) in cuotasLista" :key="index">
                                        ...
                                        <td class="text-caption px-1">{{ monedaSeleccionada }}{{
                                            redondear(cuota.importe) }}</td>
                                        <td class="text-caption px-1">{{ formatearFechaResumida(cuota.vencimiento) }}
                                        </td>
                                        <td class="px-1">
...
export default {
    ...
    props: {
        ...
        cargandoClientes: { type: Boolean, default: false }
    },
    data() {
        return {
            dialog: this.value,
    ...
    watch: {
        ...
        clienteSeleccionado: {
            async handler(nuevo) {
                if (!nuevo) return
                await this.clienteChange(nuevo)
            }
        },
        dialog(val) {
            this.$emit('input', val);
    ...
    methods: {
        customFilter(item, queryText) {
            ...
            const dni = String(item.id || item.documento || '').toLowerCase()
        ...
        async obtenerDetalleCliente(idCliente) {
            if (!idCliente) return null
            try {
                const snap = await colClientes().doc(String(idCliente)).get()
                if (!snap.exists) return null
                const data = snap.data() || {}
                return {
                    id: String(snap.id || idCliente),
                    documento: String(data.documento || snap.id || idCliente),
                    nombre: data.nombre || '',
                ...
                console.error('Error consultando cliente en Firestore:', error)
                return null
            }
        },
        async clienteChange(cliente) {
            if (cliente) {
                const idCliente = String(cliente.id || cliente.documento || '')
                this.documentoCliente = idCliente;
                this.zonaCliente = cliente.zona || 'SIN ZONA';

                this.buscandoClientes = true
                try {
                    const detalle = await this.obtenerDetalleCliente(idCliente)
                    this.clienteDataDetalle = detalle

                    if (detalle?.zona !== undefined) {
                        this.zonaCliente = detalle.zona || 'SIN ZONA'
                    }
                } finally {
        ...
        async guardar() {
            ...
            try {
                ...
                const clienteData = this.clienteDataDetalle || this.clienteSeleccionado || {}

                const cuentaData = {
                    doc_ref: this.docRef,
                    documento: String(clienteData.documento || clienteData.id || this.clienteSeleccionado.id || ''),
                    nombre: clienteData.nombre || this.clienteSeleccionado.nombre || '',
                    direccion: clienteData.direccion || '',
                    telefono: clienteData.telefono || '',
                    cliente_zona: clienteData.zona || this.zonaCliente || '',
                    empleado: '',
                    ...
                    cuenta_manual: true,
                    usuario_registro: this.$store.state.sedeActual?.codigo || ''
                };
