<template>
    <div>
        <template v-if="modo === 'tab'">
            <v-card flat color="transparent">
                <v-card-title class="px-0 pb-4 d-flex align-center">
                    <div class="text-h5 font-weight-black">Categorias</div>
                    <v-spacer />
                    <v-btn color="primary" rounded depressed @click="$emit('agregar')">
                        <v-icon left>mdi-plus</v-icon> Nueva Categoria
                    </v-btn>
                </v-card-title>
        ...
        <v-autocomplete v-else :value="localValue.nombre" :items="items" label="Categoria" outlined rounded dense
