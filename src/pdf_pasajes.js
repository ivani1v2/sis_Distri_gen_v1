...
export const pdfGenera_pasaje = (arraydatos, cabecera, medida, modo) => {
  ...
  switch (medida) {
    ...
    case "58":
      ...
      break;
    case "reserva":
      var resp = imprime_reserva(arraydatos, cabecera);
      break;
  }
};
...
function impresion80(arraydatos, qr, cabecera) {
  ...
  //----pasajeros
  /*
  console.log(arraycabe.pasajeros);
  let pasaje = arraycabe.pasajeros;
  var nuevoArray = new Array(pasaje.length);
  for (var i = 0; i < pasaje.length; i++) {
    nuevoArray[i] = new Array(4);
    nuevoArray[i][0] = pasaje[i].asiento;
    nuevoArray[i][1] = pasaje[i].numero + " - " + pasaje[i].nombre;
  }

  doc.autoTable({
    startY: linea,
    margin: { top: linea - 9, left: 1.5 },
    styles: {
      fontSize: 7.5,
      cellPadding: 0.1,
      valign: "middle",
    ...
    columnStyles: {
      ...
      1: { columnWidth: 55, halign: "left" },
    },
    theme: ["plain"],
    head: [["Asiento", "Pasajero"]],
    body: nuevoArray,
  });

  let finalYy = doc.previousAutoTable.finalY;
  linea = finalYy + 2;
*/
  linea = linea + 3.5 * texto.length;
  var texto = doc.splitTextToSize("Pasajero: ", pdfInMM - lMargin - rMargin);
  doc.text(texto, lMargin, linea, "left");
  linea = linea + 3.5 * texto.length;

  let pasaje = arraycabe.pasajeros;
  var nuevoArray = new Array(pasaje.length);
  let asientos = pasaje.map((p) => p.asiento_nom).join(", ");
  doc.setFont("Helvetica", "");
  for (var i = 0; i < pasaje.length; i++) {
    console.log(pasaje[i]);
  ...
  doc.setFontSize(8);
  doc.setFont("Helvetica", "");
  var texto = doc.splitTextToSize("Asiento: ", pdfInMM - lMargin - rMargin);
  doc.text(texto, lMargin, linea, "left");

  doc.setFont("Helvetica", "bold");
  doc.setFontSize(13);
  var texto = doc.splitTextToSize(asientos, pdfInMM - lMargin - rMargin);
  doc.text(texto, lMargin + 25, linea, "left");

  linea = linea + 4.5 * texto.length;

  doc.setFontSize(8);
  doc.setFont("Helvetica", "");
  var texto = doc.splitTextToSize("Dia: ", pdfInMM - lMargin - rMargin);
  doc.text(texto, lMargin, linea, "left");

  doc.setFont("Helvetica", "bold");
  doc.setFontSize(13);
  var texto = doc.splitTextToSize(
    moment.unix(pasaje[0].f_salida).format("dddd"),
    pdfInMM - lMargin - rMargin
  );
  doc.text(texto, lMargin + 25, linea, "left");
  linea = linea + 4.5 * texto.length;
  ...
  doc.setFontSize(8);
  doc.setFont("Helvetica", "");
  var texto = doc.splitTextToSize("Fecha Viaje: ", pdfInMM - lMargin - rMargin);
  doc.text(texto, lMargin, linea, "left");

  doc.setFont("Helvetica", "bold");
  doc.setFontSize(13);
  var texto = doc.splitTextToSize(
    moment.unix(pasaje[0].f_salida).format("DD/MM/YY"),
    pdfInMM - lMargin - rMargin
  );
  doc.text(texto, lMargin + 25, linea, "left");
  linea = linea + 4 * texto.length;
  ...
  doc.setFontSize(8);
  doc.setFont("Helvetica", "");
  var texto = doc.splitTextToSize("Hora Viaje: ", pdfInMM - lMargin - rMargin);
  doc.text(texto, lMargin, linea, "left");
  ...
  doc.setFontSize(8);
  var texto = doc.splitTextToSize(
    "CONDICIONES DEL SERVICIO DE VIAJE:\n\n" +
      "1. El pasajero deberá presentarse 30 minutos antes de la salida del bus.\n\n" +
      "2. Las postergaciones de boletos se admitirán con (4) horas de anticipación.\n\n" +
      "3. Los niños mayores de (5) años, viajarán en su propio asiento y pagarán su pasaje completo.\n\n" +
      "4. Todo menor de edad deberá viajar acompañado de sus padres o tutor familiar con autorización notarial.\n\n" +
      "5. El pasajero podrá transportar gratuitamente 10 Kg. como equipaje; el exceso será admitido sólo si hay capacidad disponible en el bus, previo pago respectivo.\n\n" +
      "6. La empresa no se responsabiliza por pérdida de equipaje en el salón del bus.",
    68
  );
...
export const imprime_reserva = (arraydatos, cabecera) => {
  var arraycabe = cabecera;
  var linea = parseInt(store.state.configImpresora.msuperior);
  var Ruc = "Ruc: " + store.state.baseDatos.ruc;
  var Direccion =
  ...
  for (var i = 0; i < array.length; i++) {
    ...
    nuevoArray[i][0] = array[i].cantidad;
    nuevoArray[i][1] = array[i].nombre;
    nuevoArray[i][2] = array[i].precio;
    nuevoArray[i][3] = parseFloat(array[i].precio * array[i].cantidad).toFixed(
      2
    );
  }
